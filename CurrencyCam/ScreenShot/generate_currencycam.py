#!/usr/bin/env python3
"""
CurrencyCam App Store Screenshot Generator
=============================================
...
# App Store sizes — only the two required: iPhone 6.5" and iPad 13"
SIZES = {
    "6.5": (1284, 2778),     # iPhone 15 Plus / 14 Pro Max
    "ipad13": (2064, 2752),  # iPad Pro 13" (M4)
}

# Slot colors (5 slots × consistent across languages)
SLOT_BG = [
    ((30, 110, 220), (20, 70, 160)),     # 1. Camera - signature blue
    ((44, 62, 80), (26, 37, 47)),        # 2. Detection - dark slate
    ((79, 121, 113), (55, 90, 83)),      # 3. Settings - teal
    ((91, 134, 229), (54, 130, 200)),    # 4. Currencies - lighter blue
    ((103, 82, 171), (66, 51, 119)),     # 5. Manual Input - purple
]

# Screenshot definitions per language
# Slots (in order): 1=Camera, 2=Detection, 3=Settings, 4=Currencies, 5=Manual Input
SCREENSHOTS = {
    "en": [
        {
            "file": "IMG_4805.PNG",
        ...
        {
            "file": "IMG_4806.PNG",
            "headline": "Adjustable Frame\nPinpoint Any Number",
            "sub": "Drag and resize the frame to detect only the amount you want",
            "bg": SLOT_BG[1],
            "text_color": WHITE,
        },
        {
        ...
        },
        {
            ...
            "sub": "Tap in any amount — see conversions to all your currencies at once",
            "bg": SLOT_BG[4],
            "text_color": WHITE,
        },
    ],
    "jp": [
        ...
        },
        {
            "file": "IMG_4812.PNG",
            "headline": "取込枠を自由に調整\n狙った数字だけ検出",
            "sub": "ドラッグ＆ピンチで枠のサイズを変更、目的の金額だけを認識",
            "bg": SLOT_BG[1],
            "text_color": WHITE,
        },
        {
    ...
    "es": [
        ...
        {
            "file": "IMG_4817.PNG",
            "headline": "Marco Ajustable\nEnfoca el Número",
            "sub": "Arrastra y ajusta el marco para detectar solo el importe que quieras",
            "bg": SLOT_BG[1],
            "text_color": WHITE,
        },
        {
        ...
        {
            "file": "IMG_4818.PNG",
            "headline": "Entrada Manual\nRápida",
            "sub": "Introduce cualquier importe — mira todas las conversiones al instante",
            "bg": SLOT_BG[4],
            "text_color": WHITE,
        },
    ],
    "ko": [
        {
            "file": "IMG_4821.PNG",
            "headline": "카메라만 비추면\n즉시 환율 환산",
            "sub": "금액을 자동 인식하여 실시간으로 통화를 변환합니다",
            "bg": SLOT_BG[0],
            "text_color": WHITE,
        },
        {
            "file": "IMG_4823.PNG",
            "headline": "프레임 크기 조절\n원하는 숫자만 인식",
            "sub": "드래그와 핀치로 프레임을 조정해 원하는 금액만 인식하세요",
            "bg": SLOT_BG[1],
            "text_color": WHITE,
        },
        {
            "file": "IMG_4825.PNG",
            "headline": "환율과 통화를\n자유롭게 설정",
            "sub": "기준 통화와 여러 환산 통화의 환율을 직접 관리하세요",
            "bg": SLOT_BG[2],
            "text_color": WHITE,
        },
        {
            "file": "IMG_4826.PNG",
            "headline": "50개 이상의\n통화 지원",
            "sub": "전 세계 주요 통화와 지역별 통화를 폭넓게 지원",
            "bg": SLOT_BG[3],
            "text_color": WHITE,
        },
        {
            "file": "IMG_4824.PNG",
            "headline": "수동 입력도\n지원",
            "sub": "키보드로 금액을 입력하면 모든 목표 통화로 즉시 환산",
            "bg": SLOT_BG[4],
            "text_color": WHITE,
        },
    ],
...
def is_cjk_char(ch):
    """Check if character is CJK/Hangul (needs CJK font)."""
    cp = ord(ch)
    return (
        (0x3000 <= cp <= 0x303F) or
        (0x3040 <= cp <= 0x309F) or
...
def main():
    print("=" * 60)
    print("  CurrencyCam App Store Screenshot Generator")
    print("=" * 60)
